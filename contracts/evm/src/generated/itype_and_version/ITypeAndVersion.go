...
// Main Binding Type for ITypeAndVersion
type ITypeAndVersion struct {
	Address []byte
	Options *bindings.ContractInitOptions
	ABI     *abi.ABI
	client  *evm.Client
	Codec   ITypeAndVersionCodec
}

type ITypeAndVersionCodec interface {
	EncodeTypeAndVersionMethodCall() ([]byte, error)
	DecodeTypeAndVersionMethodOutput(data []byte) (string, error)
}

func NewITypeAndVersion(
	client *evm.Client,
	address []byte,
	options *bindings.ContractInitOptions,
) (*ITypeAndVersion, error) {
	parsed, err := abi.JSON(strings.NewReader(ITypeAndVersionMetaData.ABI))
	if err != nil {
		return nil, err
	}
	codec, err := NewCodec()
...
func NewCodec() (ITypeAndVersionCodec, error) {
	parsed, err := abi.JSON(strings.NewReader(ITypeAndVersionMetaData.ABI))
	if err != nil {
		return nil, err
	}
...
func (c ITypeAndVersion) WriteReport(
	runtime cre.Runtime,
	report *cre.Report,
...
func (c *ITypeAndVersion) UnpackError(data []byte) (any, error) {
