...
func (c *IReserveManager) RegisterLogTrackingRequestReserveUpdate(runtime cre.Runtime, options *bindings.LogTrackingOptions[RequestReserveUpdate]) cre.Promise[*emptypb.Empty] {
	bindings.ValidateLogTrackingOptions[RequestReserveUpdate](options)
	topics, err := c.Codec.EncodeRequestReserveUpdateTopics(c.ABI.Events["RequestReserveUpdate"], options.Filters)
	if err != nil {
		return cre.PromiseFromResult[*emptypb.Empty](nil, fmt.Errorf("failed to encode topics for RequestReserveUpdate: %w", err))
	}
	padded := bindings.PadTopics(topics)
	return c.client.RegisterLogTracking(runtime, &evm.RegisterLogTrackingRequest{
		Filter: &evm.LPFilter{
			Name:          "RequestReserveUpdate-" + common.Bytes2Hex(c.Address),
			Addresses:     [][]byte{c.Address},
			EventSigs:     [][]byte{c.Codec.RequestReserveUpdateLogHash()},
...
func (c *IReserveManager) UnregisterLogTrackingRequestReserveUpdate(runtime cre.Runtime) cre.Promise[*emptypb.Empty] {
	return c.client.UnregisterLogTracking(runtime, &evm.UnregisterLogTrackingRequest{
		FilterName: "RequestReserveUpdate-" + common.Bytes2Hex(c.Address),
	})
}
