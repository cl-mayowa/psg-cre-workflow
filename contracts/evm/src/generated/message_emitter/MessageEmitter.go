...
func (c *MessageEmitter) RegisterLogTrackingMessageEmitted(runtime cre.Runtime, options *bindings.LogTrackingOptions[MessageEmitted]) cre.Promise[*emptypb.Empty] {
	bindings.ValidateLogTrackingOptions[MessageEmitted](options)
	topics, err := c.Codec.EncodeMessageEmittedTopics(c.ABI.Events["MessageEmitted"], options.Filters)
	if err != nil {
		return cre.PromiseFromResult[*emptypb.Empty](nil, fmt.Errorf("failed to encode topics for MessageEmitted: %w", err))
	}
	padded := bindings.PadTopics(topics)
	return c.client.RegisterLogTracking(runtime, &evm.RegisterLogTrackingRequest{
		Filter: &evm.LPFilter{
			Name:          "MessageEmitted-" + common.Bytes2Hex(c.Address),
			Addresses:     [][]byte{c.Address},
			EventSigs:     [][]byte{c.Codec.MessageEmittedLogHash()},
...
func (c *MessageEmitter) UnregisterLogTrackingMessageEmitted(runtime cre.Runtime) cre.Promise[*emptypb.Empty] {
	return c.client.UnregisterLogTracking(runtime, &evm.UnregisterLogTrackingRequest{
		FilterName: "MessageEmitted-" + common.Bytes2Hex(c.Address),
	})
}
